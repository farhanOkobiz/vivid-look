...
const Banner = ({ onLoadComplete }) => {
  ...
  useEffect(() => {
    const fetchBannerData = async () => {
      try {
        ...
        setLoading(false);
        if (onLoadComplete) onLoadComplete(); // Notify parent that the banner has loaded
      } catch (err) {
        setError(err);
        setLoading(false);
      }
  ...
  if (error)
    return (
      <div className="text-center text-red-500 font-semibold py-5">
                  ...
                  {currentSlide === i && (
                    <div className="w-full h-[50vh] lg:h-[90vh]">
                      <motion.img
                        ...
                        src={item?.photo}
                        alt="Vivid Look VividLook  VividLook VividLook is a Bangladeshi brand specializing in stylish and classy bags for fashionable individuals Vivid Look is one of most popular ecommerce website"
